#!/usr/bin/env python3
# Cliente unificado para sincronización de videos en Raspberry Pi con soporte HTTPS
...
from modules.devices import register_device, update_status, get_tienda
...
VERIFY_SSL = os.getenv("VERIFY_SSL", "True").lower() != "false"  # Verificación SSL por defecto es True
...
class JWTAuthManager:
    ...
    def __init__(self, server_url, username, password, token_file="token.json", verify_ssl=True):
        """
        ...
        Args:
            ...
            verify_ssl: Verificar certificados SSL (por defecto True)
        """
...
class VideoDownloaderClient:
    def __init__(self, server_url, download_path, username, password, check_interval=30, service_name="videoloop.service", verify_ssl=True):
    ...
    def download_playlist(self, playlist):
        ...
        for video in playlist.get("videos", []):
            ...
            try:
                ...
                # Incluir encabezados de autenticación y configuración SSL
                with requests.get(
                    video_url, 
                    stream=True, 
                    timeout=120, 
    ...
    async def check_for_updates(self):
        ...
        try:
            ...
            try:
                # Usar asyncio.to_thread para hacer la petición HTTP de forma asíncrona con autenticación y SSL
...
# Sobrescribir las funciones de registro y actualización de dispositivos con soporte SSL
def register_device(server_url=None, verify_ssl=None):
    """Registra el dispositivo en el servidor central con soporte SSL"""
    try:
        ...
        try:
            ...
            logger.warning(f"Error al obtener IP LAN: {e}")
            ip_address_lan = None
            
        try:
            eth0_mac = get_interface_mac('eth0')
        except Exception as e:
            logger.warning(f"Error al obtener MAC ETH0: {e}")
            eth0_mac = None
            
        try:
            ip_address_wifi = get_interface_ip('wlan0')
        except Exception as e:
            logger.warning(f"Error al obtener IP WiFi: {e}")
            ip_address_wifi = None
            
        try:
            wlan0_mac = get_interface_mac('wlan0')
        except Exception as e:
            logger.warning(f"Error al obtener MAC WLAN0: {e}")
            wlan0_mac = None
            
        try:
            tienda_id = get_tienda(ip_address_lan)
        except Exception as e:
            logger.warning(f"Error al obtener tienda: {e}")
            tienda_id = None


        # CORRECCIÓN: Usar el endpoint correcto para el registro de dispositivos
        # El endpoint es simplemente /api/devices (según el código del servidor)
        endpoint = f"{server_url}/api/devices"
        
        # Datos del dispositivo - asegurarse de que coincidan con el esquema esperado por el servidor
        device_data = {
            "device_id": device_id,
            "name": hostname,
            "model": model,
        ...
        # Eliminar cualquier campo que sea None para evitar problemas de validación
        device_data = {k: v for k, v in device_data.items() if v is not None}
        
        logger.info(f"Registrando dispositivo: {device_id}")
        logger.info(f"Endpoint de registro: {endpoint}")
        logger.debug(f"Datos del dispositivo: {device_data}")
        
        # Realizar la solicitud POST para registrar el dispositivo con verificación SSL
        response = requests.post(
            endpoint, 
            json=device_data, 
        ...
        if response.status_code in [200, 201]:
            logger.info(f"Dispositivo {device_id} registrado correctamente")
            return True
        elif response.status_code == 400 and "already registered" in response.text:
            # Si el dispositivo ya está registrado, considerarlo como éxito
...
def update_status(server_url=None, verify_ssl=None):
    """Actualiza el estado del dispositivo en el servidor central con soporte SSL"""
    try:
        server_url = server_url or SERVER_URL
        verify_ssl = VERIFY_SSL if verify_ssl is None else verify_ssl
        device_id = get_device_id()
        ...
        # Obtener información del dispositivo
        interface_lan = get_interface_ip('eth0')
        interface_wlan = get_interface_ip('wlan0')
        # Verificar si el servicio de reproducción está activo
        service_status = check_service("videoloop.service")
        
        # Endpoint para actualización de estado
        endpoint = f"{server_url}/api/devices/{device_id}/status"
        
        # Datos del estado actual
        status_data = {
            "ip_address_lan": interface_lan,
            "ip_address_wifi": intinterface_wlan,
            "status": "online",
            "service_running": service_status.get("active", False),
        ...
        #     "memory_usage": 0,
        #     "disk_usage": 0,
        #     "videoloop_status": "string",
        #     "kiosk_status": "string",
        #     "last_seen": "2025-07-08T18:35:18.563Z",
        #     "registered_at": "2025-07-08T18:35:18.563Z",
        #     "playlists": []
        # }
        
        # Realizar la solicitud PUT para actualizar el estado con verificación SSL
        ...
        # Verificar respuesta
        if response.status_code == 200:
            logger.debug(f"Estado del dispositivo {device_id} actualizado correctamente")
            return True
        else:
        ...
        logger.error(f"Error durante la actualización de estado: {e}")
        return False
...
def create_sync_router(verify_ssl=None):
    ...
    async def sync_status():
        ...
        # Crear el cliente con los parámetros globales y soporte SSL
